...
export function normalizeReview(r) {
  ...
  const cards = (Array.isArray(r.cards) ? r.cards : []).filter(c => c && typeof c === "object").map(c => ({ id: str(c.id || uid()), section: str(c.section), title: str(c.title), body: str(c.body) }));
...
export function snapshotOf(review, pilot, n) {
  ...
  review.cards.forEach(c => {
    ...
    s.cards.push({ id: c.id, title: c.title, blocks: parseBlocks(c.body).map(b => ({ id: b.id, kind: b.kind, text: b.text || "", rest: b.rest || "", items: b.items || [], head: b.head || [], rows: b.rows || [] })) });
  });
...
export function feedbackCounts(review) {
  ...
  return { open, total: review.feedback.filter(f => f.kind !== "finish").length, finished: finishes.length ? finishes[finishes.length - 1] : null };
}
...
export function cabinetMDraft() {
  ...
  return emptyReview({
    ...
    cards: [
      ...
      { id: "c10", section: S5, title: "Closing", body: "Once we have corrected this picture together, we can choose the improvements that best fit the way your firm works.\n\nIf something important is missing from these pages altogether, the comment box on this page is the place to say so.\n\n> Pressing “Finish review” below tells me you have been through the pages. It does not mean you agree with every sentence; the corrections you left are what we will work from.\n\nUzziel" }
    ]
  });
}
